...
impl DiceResultTable{
    ...
    pub fn new(name : String, table_results : Vec<String>) -> DiceResultTable{
        let mut index = 1;
        let table: HashMap<u32, String> = table_results.into_iter().map(|s|{
             let kvp = (index, s);
             index += 1;
             kvp
            }).collect();
    ...
    pub fn lookup(&self, mut face : u32) -> Result<&str, String>{
        let table_length: u32 = match self.table.len().try_into(){
            Ok(len) => len,
            Err(_) => return Err(format!["Table length is larger than the max supported by dice-tray. Are you sure you need a table this big?"])
        };
